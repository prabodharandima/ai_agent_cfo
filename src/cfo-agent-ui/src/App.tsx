...
function App() {
  ...
  return (
    <main className="chat-page">
      <header className="app-header"><div><p className="eyebrow">Executive finance workspace</p><h1>CFO AI Agent</h1></div><span className="mock-badge">Mock LLM</span></header>
      <section className="prompt-library" aria-label="Example CFO prompts"><h2>Start with a CFO question</h2><div>{examplePrompts.map((example) => <button className="prompt-button" key={example} type="button" disabled={isLoading} onClick={() => void sendMessage(example)}>{example}</button>)}</div></section>
      ...
      <form className="composer" onSubmit={handleSubmit}><label htmlFor="cfo-prompt">Ask the CFO AI Agent</label><textarea id="cfo-prompt" value={prompt} onChange={(event) => setPrompt(event.target.value)} onKeyDown={handlePromptKeyDown} placeholder="Ask a finance question..." rows={3} disabled={isLoading} /><div><span>Press Enter to send. Shift+Enter adds a line.</span><button type="submit" disabled={isLoading || !prompt.trim()}>{isLoading ? 'Working...' : 'Send'}</button></div></form>
    </main>
  )
}
