...
| `Cache:Classification:TtlSeconds` / `CACHE_LLM_CLASSIFICATION_TTL_SECONDS` | Expiry for successful validated LLM intent classifications | `300` | `CfoOrchestratorAgent` | Positive when caching is enabled |
| `Cache:Classification:PromptVersion` / `CACHE_LLM_CLASSIFICATION_PROMPT_VERSION` | Classifier prompt/template cache namespace | `v1` | `CfoOrchestratorAgent` | Nonblank; change when the classification prompt changes |
| `Cache:Classification:AllowedIntentSetVersion` / `CACHE_LLM_CLASSIFICATION_ALLOWED_INTENT_SET_VERSION` | Intent-set cache namespace | `v1` | `CfoOrchestratorAgent` | Nonblank; change when classification policy changes |
