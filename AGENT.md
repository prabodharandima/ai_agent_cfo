...
- `Cache:Enabled` bypasses all application caching when false. `Cache:UseDistributedCache` selects optional Redis backing; when false, HybridCache uses only its local memory tier. `Cache:Finance:*TtlSeconds`, `Cache:Rag:RetrievalTtlSeconds`, and `Cache:Embeddings:TtlSeconds` control operation-specific expiry. `Cache:Embeddings:Version` changes the embedding cache namespace after an embedding implementation change. `Rag:IndexVersion` invalidates RAG retrieval cache entries after an index rebuild without storing a raw question in a cache key.
