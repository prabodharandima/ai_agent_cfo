...
- `Cache:Enabled` bypasses all application caching when false. `Cache:UseDistributedCache` selects optional Redis backing; when false, HybridCache uses only its local memory tier. `Cache:Finance:*TtlSeconds` and `Cache:Rag:RetrievalTtlSeconds` control operation-specific expiry. `Rag:IndexVersion` invalidates RAG retrieval cache entries after an index rebuild without storing a raw question in a cache key.
