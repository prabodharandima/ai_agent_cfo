...
- `Cache:Enabled` bypasses all application caching when false. `Cache:UseDistributedCache` selects optional Redis backing; when false, HybridCache uses only its local memory tier. `Cache:Finance:*TtlSeconds`, `Cache:Rag:RetrievalTtlSeconds`, `Cache:Embeddings:TtlSeconds`, and `Cache:McpDiscovery:TtlSeconds` control expiry. `Cache:Embeddings:Version` and `Cache:McpDiscovery:SchemaVersion` version their cache namespaces. `Rag:IndexVersion` invalidates RAG retrieval cache entries after an index rebuild without storing a raw question in a cache key.
