...
- Financial summaries, comparisons, rankings, percentages, and forecast values are deterministic C# and SQL results. For sales summaries only, an LLM may propose an ISO date range from the user's wording; C# canonicalizes standard relative phrases and validates other ranges for format, ordering, and the non-future boundary before Finance MCP is called. An LLM never creates authoritative finance values.
...
- Preserve exactly four agents, the approved five Finance MCP tools, and the two Knowledge File MCP tools. The typed Finance MCP client selects only its fixed, configured-approved tool and supplies canonical arguments after C# validation; `IChatClient` classifies requests, proposes bounded sales-summary date ranges, and explains verified results but never selects MCP tools, endpoints, or authoritative finance values. Do not add auth, streaming, history, CQRS, MediatR, extra agents, write MCP operations, or microservices beyond the two approved hosted MCP services.
