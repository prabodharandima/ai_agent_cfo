...
- Finance MCP read results, ChromaDB retrieval results, deterministic embedding vectors, approved MCP discovery names, and successful validated LLM intent classifications are cached only through `IApplicationCache`. `HybridApplicationCache` provides memory-only local mode and optional Redis backing; Redis is configured only in `Program.cs` and Docker Compose. Cache loss must fail open to the authoritative dependency, and failures, timeouts, cancellation, prompt-risk blocks, malformed model output, deterministic fallbacks, and final responses must never be cached.
- `IChatClient` is selected at the composition root from `AI:Provider`. Ollama is the only registered provider today: its adapter, options, SDK configuration, and health check stay under `AI/Ollama`. `AgentChatMiddleware` wraps that client through the Microsoft Agent Framework-compatible chat pipeline for timing, correlation-aware safe logging, configured prompt-risk checks, and sensitive-output redaction for normal and streaming calls. It does not route business requests, select MCP tools, or map HTTP errors. Containers reach Ollama's Windows-host endpoint through `host.docker.internal`. No cloud-provider adapter is implemented.
- `AgentActivityTracing` exposes standard .NET OpenTelemetry-compatible `ActivitySource` and `Meter` signals at request, classification, specialist, LLM, Finance MCP, ChromaDB, and composition boundaries. It records only correlation ID, operation, agent, provider, model, outcome, and duration. Prompts, responses, tool arguments, retrieved text, finance data, secrets, and connection strings must never be telemetry attributes. Exporter registration remains optional and is intentionally not required for normal local execution or tests.
- Preserve exactly four agents, the approved five Finance MCP tools, and the two Knowledge File MCP tools. The typed Finance MCP client selects only its fixed, configured-approved tool and supplies deterministic canonical arguments; `IChatClient` classifies requests and explains verified results but never selects MCP tools, endpoints, or authoritative finance arguments. `POST /api/chat` remains the default response mode; the only approved streaming surface is the separate safe SSE endpoint `POST /api/chat/stream`. Do not add auth, persistent history, CQRS, MediatR, extra agents, write MCP operations, or microservices beyond the two approved hosted MCP services.
...
- `Cache:Enabled` bypasses all application caching when false. `Cache:UseDistributedCache` selects optional Redis backing; when false, HybridCache uses only its local memory tier. `Cache:Finance:*TtlSeconds`, `Cache:Rag:RetrievalTtlSeconds`, `Cache:Embeddings:TtlSeconds`, `Cache:McpDiscovery:TtlSeconds`, and `Cache:Classification:TtlSeconds` control expiry. `Cache:Embeddings:Version`, `Cache:McpDiscovery:SchemaVersion`, and `Cache:Classification` prompt and allowed-intent versions version their cache namespaces. `Rag:IndexVersion` invalidates RAG retrieval cache entries after an index rebuild without storing a raw question in a cache key.
...
- `AgentSessions:MessageLimit`, `AgentSessions:ExpirationMinutes`, and `AgentSessions:MaximumSessions` bound the in-memory session map. A session retains only prior resolved response types and optional date periods for classification context; it never retains prompts, answers, raw RAG content, credentials, authorization state, or MCP data. Sessions are local to one API process and disappear on restart.
