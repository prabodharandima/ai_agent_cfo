...
- Finance MCP read results are cached only through `IApplicationCache`. `HybridApplicationCache` provides memory-only local mode and optional Redis backing; Redis is configured only in `Program.cs` and Docker Compose. Cache loss must fail open to Finance MCP, and failures, timeouts, and cancellation must never be cached.
...
- `Cache:Enabled` bypasses all application caching when false. `Cache:UseDistributedCache` selects optional Redis backing; when false, HybridCache uses only its local memory tier. The six `Cache:Finance:*TtlSeconds` values control finance-operation expiry independently.
