...
- Finance read results and ChromaDB retrieval results use provider-neutral HybridCache caching. Compose enables Redis at `redis:6379`; local `appsettings.json` uses memory-only caching. Set `CACHE_ENABLED=false` to bypass caching. RAG keys contain only a normalized-question hash, filters, threshold, and `RAG_INDEX_VERSION`; change that version after rebuilding the index. Redis is internal, non-persistent, and never a source of truth, so a cache failure falls back to the authoritative dependency.
