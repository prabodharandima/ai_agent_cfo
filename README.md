...
- Finance read results, ChromaDB retrieval results, deterministic embedding vectors, and approved MCP discovery names use provider-neutral HybridCache caching. Compose enables Redis at `redis:6379`; local `appsettings.json` uses memory-only caching. Set `CACHE_ENABLED=false` to bypass caching. MCP discovery keys contain only fingerprints of dependency identity, endpoint, allow-list, and schema version; SDK tool objects, endpoints, arguments, results, and secrets are not cached. Redis is internal, non-persistent, and never a source of truth, so a cache failure falls back to the authoritative dependency.
