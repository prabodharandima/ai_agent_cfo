...
- Finance read results, ChromaDB retrieval results, and deterministic embedding vectors use provider-neutral HybridCache caching. Compose enables Redis at `redis:6379`; local `appsettings.json` uses memory-only caching. Set `CACHE_ENABLED=false` to bypass caching. RAG keys contain only a normalized-question hash, filters, threshold, and `RAG_INDEX_VERSION`; embedding keys contain fingerprints of the source text, generator identity, vector dimension, and `CACHE_EMBEDDINGS_VERSION`. Redis is internal, non-persistent, and never a source of truth, so a cache failure falls back to the authoritative dependency.
