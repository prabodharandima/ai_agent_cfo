# TASK-CFO-P8-004 — PostgreSQL Migrations and Seeding

## Objective

Replace SQLite with PostgreSQL inside Finance MCP.
...
Add Npgsql EF provider, migrations, constraints/indexes, deterministic seed/reset, and PostgreSQL integration tests.

## Out of scope

PostgreSQL access from API, HTTP MCP, Docker Compose, production secrets.

## Implementation requirements

Remove SQLite from Finance MCP. Preserve decimal precision/date behavior. Use environment-overridable connection strings and no committed credentials. Make seeding idempotent and non-destructive. Support explicit migration/seed commands or safe configured startup migration. Test all five tools against PostgreSQL.

## Acceptance criteria

Finance MCP has no SQLite dependency; schema is reproducible; seed is idempotent; all finance results match baseline; API gains no PostgreSQL dependency.
