...
internal sealed class TemporaryFinanceDatabase : IAsyncDisposable
{
    private TemporaryFinanceDatabase(string path, FinanceDbContext context)
    {
        Path = path;
        Context = context;
    }
    ...
    public static async Task<TemporaryFinanceDatabase> CreateAsync()
    {
        var path = System.IO.Path.Combine(System.IO.Path.GetTempPath(), $"cfo-agent-phase-1-{Guid.NewGuid():N}.db");
        ...
        return new TemporaryFinanceDatabase(path, context);
    }
